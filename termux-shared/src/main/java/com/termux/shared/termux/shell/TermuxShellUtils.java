...
public class LinuxLatorShellUtils {

    private static final String LOG_TAG = "LinuxLatorShellUtils";
            ...
            try (FileInputStream in = new FileInputStream(file)) {
                ...
                if (bytesRead > 4) {
                    if (buffer[0] == 0x7F && buffer[1] == 'E' && buffer[2] == 'L' && buffer[3] == 'F') {
                            ...
                            if (c == ' ' || c == '\n') {
                                if (builder.length() == 0) {
                                    ...
                                    if (shebangExecutable.startsWith("/usr") || shebangExecutable.startsWith("/bin")) {
                                        ...
                                        interpreter = LinuxLatorConstants.TERMUX_BIN_PREFIX_DIR_PATH + "/" + binary;
                                    }
                                    break;
                                }
                            } else {
                        ...
                        interpreter = LinuxLatorConstants.TERMUX_BIN_PREFIX_DIR_PATH + "/sh";
                    }
                }
            }
        } catch (IOException e) {
    ...
    /** Clear files under {@link LinuxLatorConstants#TERMUX_TMP_PREFIX_DIR_PATH}. */
    public static void clearLinuxLatorTMPDIR(boolean onlyIfExists) {
        ...
        if(onlyIfExists && !FileUtils.directoryFileExists(LinuxLatorConstants.TERMUX_TMP_PREFIX_DIR_PATH, false))
            return;
        ...
        if (days < 0) {
            ...
            error = FileUtils.clearDirectory("$TMPDIR",
                FileUtils.getCanonicalPath(LinuxLatorConstants.TERMUX_TMP_PREFIX_DIR_PATH, null));
            if (error != null) {
